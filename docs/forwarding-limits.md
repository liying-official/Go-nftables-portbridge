...
Go UDP uses bounded sessions and buffers. Temporary send pressure counts unsent packets as drops while retaining valid sessions; there is no unbounded retry queue. Capacity, latency and hardware offload depend on deployment. Source packages are unsigned, not publisher-signed binaries.
...
Go UDP 使用有界会话和缓冲。暂时发送压力下，未发送包计入丢弃而保留有效会话，不使用无界重试队列。容量、延迟及硬件卸载取决于部署环境。源码包未签名，不是发布者签名二进制。
